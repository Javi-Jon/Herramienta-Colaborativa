...
//      echo $numeros[1]->total;
        ...
        $estoy = false;
        do {
            if ($_SESSION['idusuario'] == $participantes[$x]['id']) {
                $estoy = true;
            } else {
                $x++;
            }
        } while ($estoy == false && $x < sizeof($participantes));

        $datos = ['proyecto' => $proyecto, 'participantes' => $participantes, 'tareas' => $tareas, 'progreso' => $numeros[1]->total, 'totales' => $restantes, 'muro' => $mensajes, 'stats' => $stats, 'yo' => $_SESSION['idusuario'], 'archivos' => $archivos, 'solicitudes' => $solicitudes, 'participo' => $estoy];
