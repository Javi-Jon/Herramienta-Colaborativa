...
class Proyecto extends BD {
    ...
    function getStatsProyecto() {
        $stats = $this->fSelectN("SELECT COUNT(asignacion.id) as stat,usuarios.username FROM `asignacion`,usuarios,proyectos,tareas WHERE iduser=usuarios.id and tareas.proyecto=proyectos.id and asignacion.idtarea=tareas.id AND proyectos.id=:idproyecto GROUP BY usuarios.id", ['idproyecto' => $this->getId()]);
        return $stats;
    }
